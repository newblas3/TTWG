...
for i in range(0, 20):  # 画网格竖线
    for j in range(0, 19):
        ...
        qi = [[q3[i, j], q3[i, j + 1]]]
        plt.plot(pi[0], qi[0], linewidth=0.5, color='r')
plt.scatter(p3, q3, s=5, color='b')    # 画散点
...
while True:
    ...
    for count in range(0, count1 + 1):  # 使网格线绘制次数与散点绘制次数一致
        ...
        for yi in By:
            v.append(yi)
        plt.cla()  # 清除上一次画的图
        t3 = np.asarray(t).squeeze()
        v3 = np.asarray(v).squeeze()
        for i in range(0, 20):  # 画网格竖线
                ti = [[t3[i, 0], t3[i, 1], t3[i, 2], t3[i, 3], t3[i, 4], t3[i, 5], t3[i, 6], t3[i, 7], t3[i, 8], t3[i, 9],
                       t3[i, 10], t3[i, 11], t3[i, 12], t3[i, 13], t3[i, 14], t3[i, 15], t3[i, 16],
                       t3[i, 17], t3[i, 18], t3[i, 19]]]
                vi = [[v3[i, 0], v3[i, 1], v3[i, 2], v3[i, 3], v3[i, 4], v3[i, 5], v3[i, 6], v3[i, 7], v3[i, 8], v3[i, 9],
                       v3[i, 10], v3[i, 11], v3[i, 12], v3[i, 13], v3[i, 14], v3[i, 15], v3[i, 16],
                       v3[i, 17], v3[i, 18], v3[i, 19]]]
                plt.plot(ti[0], vi[0], linewidth=0.5, color='r')
        for j in range(0, 20):  # 画网格横线
                ti = [[t3[0, j], t3[1, j], t3[2, j], t3[3, j], t3[4, j], t3[5, j], t3[6, j], t3[7, j], t3[8, j], t3[9, j],
                       t3[10, j], t3[11, j], t3[12, j], t3[13, j], t3[14, j], t3[15, j], t3[16, j],
                       t3[17, j], t3[18, j], t3[19, j]]]
                vi = [[v3[0, j], v3[1, j], v3[2, j], v3[3, j], v3[4, j], v3[5, j], v3[6, j], v3[7, j], v3[8, j], v3[9, j],
                       v3[10, j], v3[11, j], v3[12, j], v3[13, j], v3[14, j], v3[15, j], v3[16, j],
                       v3[17, j], v3[18, j], v3[19, j]]]
                plt.plot(ti[0], vi[0], linewidth=0.5, color='r')
        if count == count1:
            print('-' * 100)
